...
public class ClusteredServiceImpl implements ClusteredService {
  ...
  public void onSessionMessage(
    ...
    if (message.toUpperCase().startsWith(TIMER_COMMAND)) {
      long interval = Long.parseLong(message.substring(TIMER_COMMAND.length()).trim());
      scheduleTimer(++timeIdCounter, interval);
    }
  ...
  public void onTakeSnapshot(ExclusivePublication snapshotPublication) {
    ...
    buffer.putLong(Integer.BYTES, timeIdCounter);
  ...
  private void onLoadSnapshot(Image snapshotImage) {
    ...
    FragmentHandler handler =
        (buffer, offset, length, header) -> {
          ...
          timeIdCounter = buffer.getLong(offset + Integer.BYTES);
        };
  ...
  private void scheduleTimer(long correlationId, long offset) {
    final long deadlineMs = cluster.time() + offset;
